      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
          ...
          <Card className="border-destructive/20 bg-destructive/5 relative overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="text-2xl text-destructive text-center">
                Tradicional
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              {comparisonData.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between border-b border-destructive/10 pb-2 last:border-0"
                >
                  <span className="text-sm font-medium text-muted-foreground">
                    {item.feature}
                  </span>
                  <span className="text-sm font-semibold text-destructive/80 text-right">
                    {item.traditional}
                  </span>
                </div>
              ))}
          ...
          <Card className="border-primary bg-background shadow-xl relative overflow-hidden transform hover:scale-105 transition-transform duration-300">
            <div className="absolute top-0 right-0 p-0">
              <Badge className="rounded-tl-none rounded-br-none px-4 py-1 text-sm">
                Recomendado
              </Badge>
            </div>
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              {comparisonData.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between border-b border-primary/10 pb-2 last:border-0"
                >
                  <span className="text-sm font-medium text-muted-foreground">
                    {item.feature}
                  </span>
